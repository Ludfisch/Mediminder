  ...
  try {
    ...
    if (imageData) {
      if (imageData.length > 350_000) {
        return NextResponse.json({ error: "Das Packungsbild ist zu groß." }, { status: 413 });
      }
      ...
      imageType = match[1];
      if (image.length > MAX_IMAGE_BYTES || !hasExpectedImageSignature(image, imageType)) {
        return NextResponse.json(
    ...
    const result = await getPrisma().medication.updateMany({
    ...
    return NextResponse.json({ message: "Medikament aktualisiert." });
  } catch (error) {
    console.error("UPDATE_MEDICATION_ERROR", error);
    return NextResponse.json(
      { error: "Das Medikament konnte nicht aktualisiert werden." },
      { status: 500 },
    );
  }
}
